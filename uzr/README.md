...
복잡한 규칙·지식·추론 태스크에서 **실험 기준으로 열 번 물으면 아홉 번 이상은 같은 결론 쪽으로 안정적으로 수렴하는** 정도의 정합성을 목표로 설계했습니다(정확도 수치는 러프한 실험 감각입니다).
...
이미 존재하는 세부 문서(`docs/*.md`, `docs/DOCS_LURIA.md`, `LURIA_LOGGING_README.md`, `agent_lora_qnn.md`, `QNN support manual.txt` 등)는 그대로 유지하면서, 이 파일에서 큰 그림과 파일 간 관계를 모두 정리합니다.
...
- `TCodebookTokenizer` (구조 토크나이저, 기본)
  - 슬라이딩 윈도우로 구조 해시(TCodebook) 토큰을 생성하는 구조 전용 토크나이저.
  - KoEn/KoBERT는 비활성 상태이며 구조 토크나이저가 기본·유일 경로.
  - 체크포인트 vocab이 현재 토크나이저보다 작으면 vocab을 잘라 맞춰줌.

이 로직은 `chat_cli.py`의 `_pick_tokenizer_for_ckpt`와 `docs/DOCS_LURIA.md`에 기술된 “차원 불일치 로딩 폴백” 규칙을 따른 것입니다.
...
대표적인 정책 파라미터는 다음과 같습니다. (`docs/DOCS_LURIA.md`, `docs/memory.md` 참고)
...
`docs/DOCS_LURIA.md` 및 `LURIA_LOGGING_README.md`에 기술된 대로, 메모리는 여러 계층으로 구성됩니다.
...
### 6.1 구조 토크나이저(TCodebook, 기본/유일)

- TCodebook 슬라이딩 토크나이저(기본 Gt=4/Kt=256, window=8/stride=4)를 사용합니다.
- ids→텍스트 복원은 지원하지 않고, 디버그용 구조 토큰 문자열(KA07 등)만 반환합니다.
- KoEn/KoBERT 토크나이저는 비활성 상태입니다.

### 6.2 KoBERT 로컬 모델 (레거시/비활성)
...
- 사용법:
  - 기본 토크나이징에는 사용하지 않으며, 힌트/연구 목적 레거시 옵션만 유지합니다.
...
- `build_model(...)` — TCodebookTokenizer + UZRModel 생성 (구조 토크나이저만 사용)
...
7. `docs/DOCS_LURIA.md` 및 changelog — 매뉴얼에서 제안된 정책들이 코드에 어떻게 구현되었는지 추적
...
- 보다 세부적인 수식·하이퍼파라미터·튜닝 팁은 `docs/*.md`, `docs/DOCS_LURIA.md`, `LURIA_LOGGING_README.md`, `agent_lora_qnn.md`, `QNN support manual.txt`, 그리고 `changelog.md`를 함께 참고하시기 바랍니다.
