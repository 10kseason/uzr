...
def main():
    ...
    for step in pbar:
        ...
        # Quality gate check (1k-2k relaxed zone)
        # Check if current update passes quality standards
        gate_passed = True
        ...
        if not use_autonomous_abstain:
            ...
            if in_gray_zone:
                ...
                tau_r_adjusted = tau_r.item() + (1.0 - abstain_cap) * 0.5
                if seq_len > 64:
        ...
        # Coverage-aware LR adjustment (lacomi.txt prescription)
        ...
        coverage_estimate = 1.0 - abstain_ratio
        lr_mult = lr_schedule(step)

        # Apply coverage-based adjustment
        if coverage_estimate < 0.4:
        ...
        with torch.no_grad():
            ...
            if should_commit:
                ...
                if hasattr(mem, "add_with_policy"):
                    mem.add_with_policy(key, val, step, meta={"lang": int(lang_idx), "desc": desc, "composite_score": float(composite_score), "luria_intent_force": intent_force})
                else:
