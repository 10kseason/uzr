...
def init_from_retrieval_multi(mem: Optional[CompressedMemory], enc_avg: torch.Tensor, z_rule_ref: torch.Tensor,
                               z_think_ref: torch.Tensor, lang_id: int, topk: int = 4) -> Tuple[torch.Tensor, torch.Tensor, dict]:
    retrieval_info = {"retrieved_count": 0, "matched_lang": False, "distances": [], "metas": []}

    if mem is None:
        return z_rule_ref.new_zeros(z_rule_ref.shape), z_think_ref.new_zeros(z_think_ref.shape), retrieval_info
    items = mem.retrieve(enc_avg, topk=topk)
    if not items:
        return z_rule_ref.new_zeros(z_rule_ref.shape), z_think_ref.new_zeros(z_think_ref.shape), retrieval_info

    retrieval_info["distances"] = [float(it.dist) if hasattr(it, 'dist') else 0.0 for it in items]
    retrieval_info["metas"] = [it.val.get("meta", {}) for it in items]
    ...
    def collect(candidates, match_lang: bool):
        ...
        if rules:
            return (torch.stack(rules, dim=0).mean(dim=0), torch.stack(thinks, dim=0).mean(dim=0), len(rules))
        return None

    match = collect(items, match_lang=True)
    if match is not None:
        retrieval_info["matched_lang"] = True
        retrieval_info["retrieved_count"] = match[2]
        return match[0], match[1], retrieval_info

    match = collect(items, match_lang=False)
    if match is not None:
        retrieval_info["matched_lang"] = False
        retrieval_info["retrieved_count"] = match[2]
        return match[0], match[1], retrieval_info
...
def main():
    ...
    ap.add_argument("--summary_every", type=int, default=1)
    ...
    gen_fields = [
        ...
        'input','target','pred',
        'ce_query','conf_context','zslow_rule_l1','zslow_think_l1','zlang_norm','mem_items','timestamp'
    ]
    # Resolve absolute path and open line-buffered
    import os, atexit
    ...
    # Detailed step log (one row per inner step)
    step_fields = [
        'turn','inner_step','lang','lang_id','rule_desc',
        'loss_c','conf','step_lr',
        'grad_rule_norm','grad_think_norm',
        'zfast_rule_l1','zfast_think_l1',
        'zfast_rule_norm','zfast_think_norm',
        'ztotal_rule_l1','ztotal_think_l1',
        'timestamp'
    ]
    steplog_path = os.path.abspath("detailed_step_log.csv")
    print(f"[steplog] Writing detailed step log to: {steplog_path}")
    scsv = open(steplog_path, 'w', newline='', encoding='utf-8', buffering=1)
    sw = csv.DictWriter(scsv, fieldnames=step_fields)
    sw.writeheader()
    atexit.register(lambda: (scsv.flush(), scsv.close()))

    # Retrieval log (one row per turn)
    retr_fields = [
        'turn','lang','lang_id','rule_desc',
        'retrieved_count','matched_lang','avg_distance',
        'zfast_rule_init_l1','zfast_think_init_l1',
        'retrieved_metas','timestamp'
    ]
    retrlog_path = os.path.abspath("retrieval_log.csv")
    print(f"[retrlog] Writing retrieval log to: {retrlog_path}")
    rcsv = open(retrlog_path, 'w', newline='', encoding='utf-8', buffering=1)
    rw = csv.DictWriter(rcsv, fieldnames=retr_fields)
    rw.writeheader()
    atexit.register(lambda: (rcsv.flush(), rcsv.close()))

    # Sample log (one row per turn with all context & query samples)
    sample_fields = [
        'turn','lang','lang_id','rule_desc',
    ...
    samplelog_path = os.path.abspath("sample_log.csv")
    print(f"[samplelog] Writing sample log to: {samplelog_path}")
    smcsv = open(samplelog_path, 'w', newline='', encoding='utf-8', buffering=1)
    smw = csv.DictWriter(smcsv, fieldnames=sample_fields)
    smw.writeheader()
    atexit.register(lambda: (smcsv.flush(), smcsv.close()))

    # Threshold log (soft threshold effects per turn)
    thresh_fields = [
        'turn','lang','lang_id','rule_desc',
        'zfast_rule_l1_before','zfast_rule_l1_after','zfast_rule_sparsified',
        'zfast_think_l1_before','zfast_think_l1_after','zfast_think_sparsified',
        'zslow_rule_l1_before','zslow_rule_l1_after','zslow_rule_sparsified',
        'zslow_think_l1_before','zslow_think_l1_after','zslow_think_sparsified',
        'timestamp'
    ]
    threshlog_path = os.path.abspath("threshold_log.csv")
    print(f"[threshlog] Writing threshold log to: {threshlog_path}")
    tcsv = open(threshlog_path, 'w', newline='', encoding='utf-8', buffering=1)
    tw = csv.DictWriter(tcsv, fieldnames=thresh_fields)
    tw.writeheader()
    atexit.register(lambda: (tcsv.flush(), tcsv.close()))
    ...
    for t in range(args.turns):
        ...
        lang_idx = resolve_lang_idx(lang_key, model.num_langs)
        z_fast_rule_0, z_fast_think_0, retrieval_info = init_from_retrieval_multi(mem, enc_avg, z_slow_rule, z_slow_think, lang_idx)
        ...
        avg_dist = sum(retrieval_info["distances"]) / len(retrieval_info["distances"]) if retrieval_info["distances"] else 0.0
        rw.writerow({
            'turn': t + 1,
            'lang': lang_key,
            ...
            'matched_lang': retrieval_info["matched_lang"],
            'avg_distance': round(avg_dist, 6),
            'zfast_rule_init_l1': round(float(torch.sum(torch.abs(z_fast_rule_0)).cpu().item()), 4),
            'zfast_think_init_l1': round(float(torch.sum(torch.abs(z_fast_think_0)).cpu().item()), 4),
            'retrieved_metas': json.dumps(retrieval_info["metas"], ensure_ascii=False),
            'timestamp': now_ts,
        })
        rcsv.flush()
        ...
        smw.writerow({
            ...
            'lang_id': int(lang_idx),
            'rule_desc': desc,
            'context_inputs': json.dumps([x for x, _ in C], ensure_ascii=False),
            'context_targets': json.dumps([y for _, y in C], ensure_ascii=False),
            'query_inputs': json.dumps([x for x, _ in Q], ensure_ascii=False),
            'query_targets': json.dumps([y for _, y in Q], ensure_ascii=False),
            'timestamp': now_ts,
        })
        smcsv.flush()

        conf_val = 0.0
        for inner_step_idx in range(args.inner_steps):
            ...
            sw.writerow({
                ...
                'lang_id': int(lang_idx),
                'rule_desc': desc,
                'loss_c': round(float(loss_c.item()), 6),
                'conf': round(conf_val, 6),
                'step_lr': round(step, 6),
                'grad_rule_norm': round(float(torch.norm(grad_rule).cpu().item()), 6),
                'grad_think_norm': round(float(torch.norm(grad_think).cpu().item()), 6),
                'zfast_rule_l1': round(float(torch.sum(torch.abs(z_fast_rule)).cpu().item()), 4),
                'zfast_think_l1': round(float(torch.sum(torch.abs(z_fast_think)).cpu().item()), 4),
                'zfast_rule_norm': round(float(torch.norm(z_fast_rule).cpu().item()), 6),
                'zfast_think_norm': round(float(torch.norm(z_fast_think).cpu().item()), 6),
                'ztotal_rule_l1': round(float(torch.sum(torch.abs(z_rule_total)).cpu().item()), 4),
                'ztotal_think_l1': round(float(torch.sum(torch.abs(z_think_total)).cpu().item()), 4),
                'timestamp': now_ts,
            })
            scsv.flush()
        ...
        with torch.no_grad():
            # Track z_fast before/after soft threshold
            zfast_rule_before = z_fast_rule.clone()
            zfast_think_before = z_fast_think.clone()
            zfast_rule_l1_before = float(torch.sum(torch.abs(zfast_rule_before)).cpu().item())
            zfast_think_l1_before = float(torch.sum(torch.abs(zfast_think_before)).cpu().item())

            z_fast_rule = soft_threshold(z_fast_rule, lam_rule * 0.5)
            z_fast_think = soft_threshold(z_fast_think, lam_think * 0.5)

            zfast_rule_l1_after = float(torch.sum(torch.abs(z_fast_rule)).cpu().item())
            zfast_think_l1_after = float(torch.sum(torch.abs(z_fast_think)).cpu().item())
            zfast_rule_sparsified = int(torch.sum((zfast_rule_before != 0) & (z_fast_rule == 0)).cpu().item())
            zfast_think_sparsified = int(torch.sum((zfast_think_before != 0) & (z_fast_think == 0)).cpu().item())
            ...
            # Track z_slow before/after update and soft threshold
            zslow_rule_before = z_slow_rule.clone()
            zslow_think_before = z_slow_think.clone()
            zslow_rule_l1_before = float(torch.sum(torch.abs(zslow_rule_before)).cpu().item())
            zslow_think_l1_before = float(torch.sum(torch.abs(zslow_think_before)).cpu().item())
            ...
            zslow_rule_l1_after = float(torch.sum(torch.abs(z_slow_rule)).cpu().item())
            zslow_think_l1_after = float(torch.sum(torch.abs(z_slow_think)).cpu().item())
            zslow_rule_sparsified = int(torch.sum((zslow_rule_before != 0) & (z_slow_rule == 0)).cpu().item())
            zslow_think_sparsified = int(torch.sum((zslow_think_before != 0) & (z_slow_think == 0)).cpu().item())

            # Log threshold effects
            tw.writerow({
                'turn': t + 1,
                'lang': lang_key,
                'lang_id': int(lang_idx),
                'rule_desc': desc,
                'zfast_rule_l1_before': round(zfast_rule_l1_before, 4),
                'zfast_rule_l1_after': round(zfast_rule_l1_after, 4),
                'zfast_rule_sparsified': zfast_rule_sparsified,
                'zfast_think_l1_before': round(zfast_think_l1_before, 4),
                'zfast_think_l1_after': round(zfast_think_l1_after, 4),
                'zfast_think_sparsified': zfast_think_sparsified,
                'zslow_rule_l1_before': round(zslow_rule_l1_before, 4),
                'zslow_rule_l1_after': round(zslow_rule_l1_after, 4),
                'zslow_rule_sparsified': zslow_rule_sparsified,
                'zslow_think_l1_before': round(zslow_think_l1_before, 4),
                'zslow_think_l1_after': round(zslow_think_l1_after, 4),
                'zslow_think_sparsified': zslow_think_sparsified,
                'timestamp': now_ts,
            })
            tcsv.flush()
        ...
        if (t + 1) % args.summary_every == 0 or t < 5:
            med = statistics.median(ce_hist) if ce_hist else float("nan")
            mean = sum(ce_hist) / len(ce_hist) if ce_hist else float("nan")
            print(f"[turn {t + 1:3d}] CE={ce_q:.4f} (mean/med: {mean:.3f}/{med:.3f}) | "
                  f"z_rule_L1={z_rule_l1:.1f} z_think_L1={z_think_l1:.1f} | "
                  f"mem={mem_item_count} retr={retrieval_info['retrieved_count']} | "
                  f"conf={conf_val:.3f} | "
                  f"lang={lang_key}({lang_idx}) | "
                  f"sparsified(rule/think)={zslow_rule_sparsified}/{zslow_think_sparsified}")
            print(f"         rule: '{desc}'")
