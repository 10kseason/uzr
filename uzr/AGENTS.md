...
- 문서 참고 경로: `docs/` 폴더를 기본으로 사용합니다. 모델/메모리 핵심 요약은 `docs/uzr model arc-2.txt`, 매뉴얼 반영 사항은 `docs/DOCS_LURIA.md`, 토크나이저 메모는 `docs/memory for Tokernizer.txt`, 인프라 요약은 `docs/Infra memory for codex or ai agents.txt`를 우선 확인하세요. 문서가 커질 경우 `docs/` 내에서 파일을 분리하고 서로 링크를 남깁니다.
...
- 크기 관리: 파일 크기가 30KB를 넘으면 `codex.memory.ch2.json` 같은 별도 챕터 파일을 만들어 이어서 기록하고, 두 파일 모두를 읽어야 합니다(기존 파일에도 보조 챕터 위치를 명시). 현재 크기가 작을 때는 단일 파일을 유지합니다.
- 권장 스키마(요약):
```json
{
  "version": 1,
